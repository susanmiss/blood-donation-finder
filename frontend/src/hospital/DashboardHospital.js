...
import HospitalAvatar from '../images/city-hospital.png'
...
class DashboardHospital extends Component {
    ...
    componentDidMount() {
        ...
        this.listByHospital(hospitalId)
            .then(data => {
                if (data.error) {
            ...
            <div className="container mt-5 text-center">
                ...
                <div className="row mt-5">
                    ...
                    <div className="col-md-6">
                        <img src={HospitalAvatar} alt={hospital.hospitalName} style={{ width: '80%', heigh: '10vw' }} />
                    </div>

                    <div className="col-md-6">

                        <h2>{`Welcome ${hospital.hospitalName}!`}</h2>
                        <p>{`Joined  ${new Date(this.state.hospital.created).toDateString()}`}</p>
                        ...
                        {isAuthenticated().hospital && isAuthenticated().hospital._id === this.state.hospital._id ? (

                            <div className="d-inline-block mt-5">

                                <Link to={`/hospital/edit/${hospital._id}`} className="btn btn-raised btn-info mr-2">
                                    Edit Profile
                                </Link>
                            ...
                            return (
                                <div className="card col-md-12 m-3" key={i}>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <img src={HospitalAvatar} style={{ width: "50px" }} className="rounded mx-auto d-block" />
                                        <p className="card-text float-left">Message: {post.body}</p>
                                        <br />
                                        <p className="card-text float-left">Quantity: {post.quantity}</p>
                                        <br />
                                        <p className="font-italic mt-3">Posted by <Link to={`/dashboard-hospital/${posterId}`} style={{color: "rgb(234, 181, 178)"}}>{posterName}</Link> on {new Date(post.created).toDateString()}  </p>
