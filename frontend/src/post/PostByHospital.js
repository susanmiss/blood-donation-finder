               ...
               return (
                <div className="card col-md-12 m-3" key={i}>
                    ...
                    <div className="card-body">
                        ...
                        <img src={DefaultAvatar}  style={{width: "50px"}} className="float-right" />    
                        <p className="card-text float-left">Message: {post.body}</p>
                        <br/>
                        <p className="card-text float-left">Quantity: {post.quantity}</p>
                        <br />
                        <p className="font-italic mt-3">Posted by <Link to={`/dashboard-hospital/${posterId}`} className="secondary">{posterName}</Link> on {new Date(post.created).toDateString()}  </p>
