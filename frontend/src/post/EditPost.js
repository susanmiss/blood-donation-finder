...
class EditPost extends Component {
  ...
  componentDidMount() {
    ...
    this.init(postId);
    this.setState({
      hospital: isAuthenticated().hospital,
    });
  }
  ...
  clickSubmit = (event) => {
    ...
    if (this.isvalid()) {
      ...
      this.update(postId, token, post, error, hospitalId).then((data) => {
        if (data.error) {
          this.setState({
            error: data.error,
          });
  ...
  editPostForm = (title, body, quantity) => (
    <form>
      ...
      <button
        ...
        style={{
          backgroundColor: "rgb(234, 181, 178)",
        }}
      >
        Update Post{" "}
      </button>{" "}
  ...
  render() {
    ...
    return (
      <div className="container">
        ...
        {/* {JSON.stringify(this.state)} */} <h2> Update your Post </h2>{" "}
